"""Base modules for quantized layers."""
import torch
...
__all__ = ['Qmodes',  '_Conv2dQ', '_LinearQ', '_LinearQ_v2', '_ActQ',
           'truncation', 'get_sparsity_mask', 'FunStopGradient', 'round_pass', 'grad_scale', '_ConvTranspose1dQ']
...
def get_sparsity_mask(param, sparsity):
    ...
    threshold = bottomk.data[-1]  # Largest value among the weights selected for pruning.
...
class _ConvTranspose1dQ(nn.ConvTranspose1d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, output_padding=0, groups=1, bias=True, dilation=1, **kwargs_q):
        super(_ConvTranspose1dQ, self).__init__(in_channels, out_channels, kernel_size, 
        ...
        if self.q_mode == Qmodes.kernel_wise:
            # Quantize independently per output channel.
    ...
    def extra_repr(self):
        s_prefix = super(_ConvTranspose1dQ, self).extra_repr()
